...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            // SP == space, CRLF == \r, \n
            // request-target == absoluate-path [ "?" query]
            // absolute-path (no ?) must start with /
            // ensure that the request-line is consistent
            ...
            requestLine[METHOD] = strtok(lineToken, " ");
            ...
            // if method is not GET = 405 Method Not Allowed
            ...
            // if request-target != begin with /, 501 Not Implemented
            
            if (requestLine[REQUEST_TARGET] != '/')
            {
                error(501);
                continue;
            }
            
            // if request-target contains ", 400 Bad Request
            
            if (strchr (strchr(requestLine[REQUEST_TARGET], '\"') != NULL)
            {
                error(400);
                continue;
            }
            
            // if HTTP-version is not HTTP/1.1, 505 HTTP Version Not Supported
            ...
            // if absolute-path does not contain a ., 501 Not Implemented
            ...
            // redefine query to contain query from request target. if
            // request target is absent, string should be ""
            char *qstart = strchrnul(requestLine[REQUEST_TARGET], '?');
            char *qend = strchr(requestLine[REQUEST_TARGET], '\0');
            char query[(qend - qstart < 1) ? 1 : qend - qstart)];
            memcpy(query, (strncmp(qstart, "\0", 1) == 0) ? qstart : qstart + 1,
                (qend - qstart < 1) ? 1 : qend - qstart);

            qstart = strchrnul(requestLine[REQUEST_TARGET], "?");
            char absolutePath[qstart - requestLine[REQUEST_TARGET] + 1];
            strncpy(absolutePath, requestLine[REQUEST_TARGET],
                qstart - requestLine{REQUEST_TARGET]);
            
            absolutepath[qstart - requestLine[REQUEST_TARGET]] = '\0';
            ...
            // the last substring after the last .
            qend = strchr(absolutePath, '\0');
            char extension[qend - extInd];
            memcpy(extension, extInd + 1, qend - extInd);
            
            
            // TODO: concatenate root and absolute-path
            // redefine path to contain the concatenation of root and absolute-path
            char path[strlen(root) + strlen(absolutePath) + 1];
            strcpy(path, root);
            strcat(path, absolutePath);
            
            // TODO: ensure path exists
            // respond with 404 Not Found if not
            ...
            if (strcasecmp("php", extension) == 0)
            {
                ...
                // respond to client
                // after loading, responds to browser with
                //
...
const char* lookup(const char* extension)
{
    if (extension != NULL)
    {
        // return text/css if extension is css
        if (strcasecmp("css", extension) == 0)
        {
            return "text/css";
        }
        // text/html if extension is html
        ...
        // text/javascrip if extension is js
